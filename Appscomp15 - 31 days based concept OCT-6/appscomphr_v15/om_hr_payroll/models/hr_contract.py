...
class HrContract(models.Model):
    ...
    def clear_contract_amount_setup(self):
        for contract in self:
            if contract.ctc:
                ...
                contract.pf_amount = 0.00
                contract.pf_amount_second = 0.00
    ...
    @api.onchange('amount_settlment_diff', 'wage', 'house_rent_allowance',
                  ...
                  'tds','professional_tax', 'esi', 'pf_amount','esi_second','pf_amount_second')
    def _onchange_amount_settlment_diff(self):
        ...
        if self.ctc:
            ...
            self.contract_deduction_settlement = self.pf_amount + \
                                                 self.pf_amount_second + \
                                                 self.esi + \
                                                 self.esi_second + \
                                                 self.tds + \
