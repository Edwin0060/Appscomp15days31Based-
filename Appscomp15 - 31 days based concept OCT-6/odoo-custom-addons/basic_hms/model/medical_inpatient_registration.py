...
class medical_inpatient_registration(models.Model):
    ...
    patient_id = fields.Many2one('medical.patient', string="Patient ID", required=True)
    ...
    room_id = fields.Many2one('hospital.room', string='Room',
                              domain="[('floor_id', '=', ward_id)]")
    room_charge = fields.Float(related='room_id.list_price', store=True, string='Room Charges')
