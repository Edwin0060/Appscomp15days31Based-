...
class ReportTestCheckin(models.AbstractModel):
    _name = "report.hotel_extended.report_checkin_qweb"
    ...
    def _get_report_values(self, docids, data):
        ...
        print ('>>>>>>>>>>>>>>>>>>Report print inNNNNN', _get_room_type, _get_room_nos, get_checkin)
        return {
            "doc_ids": docids,
...
class ReportTestCheckout(models.AbstractModel):
    _name = "report.hotel_extended.report_checkout_qweb"
...
class ReportTestMaxroom(models.AbstractModel):
    _name = "report.hotel_extended.report_maxroom_qweb"
    ...
    def _get_room_used_detail(self, date_start, date_end):
        ...
        for room in hotel_room_obj.sudo().search([]):
            counter = 0
            details = {}
            ...
            if counter >= 1:
                details.update({
                    ...
                    "category": room.room_categ_id.name
                })


                room_used_details.append(details)
            print ('room_used_details>>>>>>>>>', room_used_details)
...
class ReportRoomReservation(models.AbstractModel):
    _name = "report.hotel_extended.report_room_reservation_qweb"
    _description = "Auxiliar to get the room report"
