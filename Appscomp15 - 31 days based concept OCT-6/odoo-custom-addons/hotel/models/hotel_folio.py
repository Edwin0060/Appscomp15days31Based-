...
class FolioRoomLine(models.Model):
    ...
    folio_id = fields.Many2one("hotel.folio", "Reservation Number", ondelete="cascade")
...
class HotelFolio(models.Model):
    ...
    hours_type = fields.Selection(
        [
            ("twelve_hours", "12 Hours"),
            ("twenty_hours", "24 Hours"),
        ],
    ...
    invoice_folio_count = fields.Integer(string="Invoice Count", compute='_compute_folio_count')
    identity_type = fields.Many2one('hotel.identity.type', string='Identity Type')
    ...
    def _compute_folio_count(self):
        self.invoice_folio_count = self.env['account.move'].sudo(). \
            search_count([('payment_reference', '=', self.name), ('move_type', '=', 'out_invoice')])
        ...
        for folio in self:
            ...
            for service_lines in folio.service_line_ids:
                service_vals = (0, 0, {
                    ...
                    'account_id': service_lines.product_id.property_account_income_id.id
                    if service_lines.product_id.property_account_income_id
    ...
    def get_boutique_invoice_advance_payment(self):
        ...
        return {
            'name': _('Hotel Guest Advance Payment'),
    ...
    def invoice_button_view(self):
        ...
        return {
            'name': _('Guest Reservation Invoice'),
            'res_model': 'account.move',
            'type': 'ir.actions.act_window',
            'view_mode': 'tree,form',
            'views': [(tree_view.id, 'tree'), (form_view.id, 'form')],
            'domain': [('payment_reference', '=', self.name), ('move_type', '=', 'out_invoice')],
        }
    ...
    def action_done(self):
        ...
        for record in self :
            invoice_valid = self.env["account.move"].search([("payment_reference", "=", record.name)])
            if invoice_valid:
                if invoice_valid.payment_state != 'paid':
                    raise ValidationError(_(' Alert!!.. Your Payment is Not Completed Please Completed it..'))
        for rec in self.room_line_ids:
            rooms = self.env["hotel.room"].search([("name", "=", rec.product_id.name)])
            rooms.write({"isroom": True})
...
class HotelFolioLine(models.Model):
    ...
    folio_id = fields.Many2one("hotel.folio", "Reservation", ondelete="cascade")
    checkin_date = fields.Datetime("Check In", required=True)
    checkout_date = fields.Datetime("Check Out", required=True)
    day = fields.Char(string="Days", required=True)
    ...
    @api.onchange('product_uom_qty', 'children', 'product_id' , 'price_subtotal' , 'price_subtotal')
    def capacity_validation(self):
        for rec in self:
            if rec.product_uom_qty:
                count_person = rec.product_uom_qty + rec.children
                rooms = self.env["hotel.room"].search([("name", "=", rec.product_id.name)])
                if rooms:
                    if count_person > rooms.capacity:
                        raise ValidationError(_(' Alert!!.. Room Capacity is Greater then Adults & children'))


        # count = self.children + self.product_uom_qty
        # if int(self.price_unit) == int(count):
        #     pass
        # if int(self.price_unit) < int(count):
        #     raise ValidationError(_(' Alert!!.. Room Capacity is Greater then Adults & children'))
...
class HotelIdentityType(models.Model):
    _name = "hotel.identity.type"
    _description = "Hotel Identity Type"
