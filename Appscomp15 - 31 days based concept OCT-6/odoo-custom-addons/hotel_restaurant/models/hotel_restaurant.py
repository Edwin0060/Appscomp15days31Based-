...
class HotelRestaurantReservation(models.Model):
    ...
    def hotel_management_table_cancel(self):
        ...
        return {
            ...
            'view_mode': 'form',
            'res_id': view_id.id,
            'view_id': self.env.ref('hotel_restaurant.hotel_management_table_cancel_remarks_wizard', False).id,
            'target': 'new',
        }
...
class HotelRestaurantOrder(models.Model):
    ...
    table_order_cancel_remarks = fields.Text(string='Table Order Cancel Remarks')
    table_order_cancel_remarks_2 = fields.Text(string='Table Order Cancel Remarks')

    def table_order_cancel(self):
        view_id = self.env['table.order.cancel']
        return {
            'type': 'ir.actions.act_window',
            'name': 'Hotel Management Table Order Cancel Remarks',
            'res_model': 'table.order.cancel',
            'view_type': 'form',
            'view_mode': 'form',
            'res_id': view_id.id,
            'view_id': self.env.ref('hotel_restaurant.hotel_management_table_cancel_remarks_wizard', False).id,
